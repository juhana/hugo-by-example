...
the [standard library](standard_library), in alphabetical
order. The "order defined" field shows the order properties are defined
by the compiler or in [hugolib.h](hugolib.h). With the
...
unless [aliased](alias) to another property. Note that you
...
[Directions](Direction) out of rooms are also properties.
They can hold the [object](object) that is north/south/etc
from the current room. When used as is, these properties are only used
by the [room](Rooms) object, but they are often
[aliased](alias) to other properties when used with other
...
Defining a property is very similar to an [alias](Alias):

    property <property name> <default value>

The default value can be either a [constant](Constants) or a
[dictionary word](Dictionary_Words). If an object does not
...
(especially when used with [constants](Constants)). For
